...
var _ = Describe("giterminism config", func() {
	...
	DescribeTable("",
		...
		Entry("the giterminism config not committed", entry{
			addConfig:            true,
			expectedErrSubstring: "unable to read the werf giterminism config: the file 'werf-giterminism.yaml' must be committed",
		}),
		...
		Entry("the giterminism config changed after commit", entry{
			...
			expectedErrSubstring:    "unable to read the werf giterminism config: the file 'werf-giterminism.yaml' changes must be committed",
		}),
	)
})
