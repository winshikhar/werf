...
func parseWerfConfig(ctx context.Context, tmpl *template.Template, manager giterminism.Manager, relWerfConfigPath string) error {
	configData, err := manager.FileReader().ReadConfig(ctx, relWerfConfigPath)
	if err != nil {
		return err
	}
...
func parseWerfConfigTemplatesDir(ctx context.Context, tmpl *template.Template, manager giterminism.Manager, customWerfConfigTemplatesDirRelPath string) error {
	return manager.FileReader().ReadConfigTemplateFiles(ctx, customWerfConfigTemplatesDirRelPath, func(templatePathInsideDir string, data []byte, err error) error {
		if err != nil {
			return err
		}
